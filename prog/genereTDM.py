...
import re
import hjson
version="0.8" #modifié le 20241011 "\..." -> r"/..." pour python 3.13
#----------------------------------------------------------------------------
...
#        print(f"DBG (trouvelisteclassée): {elmodif=}",end="")
#        print(f"->[{trans[elmodif]}]")
...
#    print(f"{racineSite=}")
    ...
    fichierSortie=re.sub(r"/[^/]+/\.\./|/\./",r"/",fichierSortie.replace(r"\\",r"/"))
    dossierDocuments=os.path.normcase(os.path.join(args.root,os.path.normcase(args.documents)))
    dossierDocuments=re.sub(r"/[^/]+/\.\./|/\./",r"/",dossierDocuments.replace(r"\\",r"/"))
...
#    print(f"{fichierSortie=}")
    ...
    fichierFinal=re.sub(r"/[^/]+/\.\./|/\./",r"/",fichierFinal.replace(r"\\",r"/"))
