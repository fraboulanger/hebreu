...
import io
import re
version="0.6" #modifié le 20241011 "\..." -> r"/..." pour python 3.13
...
#                else:
#                    cible = os.path.join(DossierHtmlMEF,sub_path)
#                    print(f"deplace {full_path} -> {cible}")
#                    os.replace(full_path,cible)
        return
...
#        print(pretty_html)
        ...
        with open(os.path.join(dosSor,fichier), "w", encoding="UTF-8") as f:
            f.write(pretty_html)
        os.remove(os.path.join(dosEnt,fichier))

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("-r", "--root", help="root of file tree", default=r"../htmlR")
    parser.add_argument("-nf", "--nouvHTML", help="nouvelle racine des fichiers", default=r"../html")
    parser.add_argument("-p", "--prog", help="emplacement des fichiers prog", default=r"/prog")
    parser.add_argument("-xn", "--exclude_name", nargs='*', help="exclude name", default=[r"\.bak$",r"\.png$",r"^Code",r"\.pdf",r"\.gif$",r"\.py$"])
    args = parser.parse_args()
#
    args.root=os.path.abspath(r"/".join([os.getcwd(),args.root]))
    args.nouvHTML=os.path.abspath(r"/".join([os.getcwd(),args.nouvHTML]))
    if not os.path.isdir(args.nouvHTML):
        os.makedirs(args.nouvHTML)
#
    debug=False
    monSite=TraverseSite(args)
    # deplacement des fichiers non html de args.root dans args.nouvHTML
    for el in os.listdir(args.root):
         os.replace(r"/".join([args.root,el]),r"/".join([args.nouvHTML,el]))
    os.rmdir(args.root)
